...
int main() {
  ...
  while (true) {
    ...
    if (absolute_time_diff_us(last_screen_update_time, get_absolute_time()) >
        200000) {
      update_display();
...
void start_stop_button_handler() {
  if (absolute_time_diff_us(last_startstop_int_time, get_absolute_time()) <
      500000) {
    return;
  }
...
void setting_button_handler() {
  ...
  if (absolute_time_diff_us(last_setting_int_time, get_absolute_time()) <
      500000) {
    return;
  }
...
int format_first_line(char *buffer, int buflen) {
  int n =
      snprintf(buffer, buflen, "%c%3s(%3d)%cT:%d",
